...
export function activate(context: vscode.ExtensionContext) {
    ...
    const showStreakCommand = vscode.commands.registerCommand('commit-tracker.showStreak', () => {
        const streak = context.globalState.get<number>('commitStreak') || 0;
...
async function checkCommitsToday(context: vscode.ExtensionContext, manualCheck: boolean) {
    ...
    try {
        ...
        const lastCommitDate = context.globalState.get<string>('lastCommitDate');
        let currentStreak = context.globalState.get<number>('commitStreak') || 0;

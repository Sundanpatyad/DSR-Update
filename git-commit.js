...
async function main() {
  ...
  const gitStash = await askQuestion(
    "1. Which files should be committed? (Enter . for all or list files, e.g., file1.js file2.js): ",
  );

  const filesToCommit = gitStash?.trim() ? gitStash.trim() : ".";

  const commitMsg = await askQuestion("2. Enter commit message (Required): ");
  if (!commitMsg.trim()) {
    console.error("Error: Commit message is compulsory.");
    process.exit(1);
  }

  const moduleName = await askQuestion("3. Enter Module Name (Required): ");
  if (!moduleName.trim()) {
  ...
  const taskType = await askQuestion(
    "4. Enter Task Type (Required): (F/f = Feature, B/b = Bug, R/r = Refactor)",
  );
  if (!taskType.trim()) {
  ...
  if (["bug", "refactor"].includes(tasktype)) {
    problemStmt = await askQuestion(
      "4(i). Problem Statement (e.g., API null issue, UI padding issue, App crash on click etc.): (Required) ",
    );
    if (!problemStmt.trim()) {
      console.error(
